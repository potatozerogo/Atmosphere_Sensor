...
int8_t Voc_Set_Read_Mod(void)
{
	...
	HAL_UART_Transmit(&huart3,Command_Buff,9,10000);
	
	__HAL_UART_CLEAR_FLAG(&huart3,UART_FLAG_RXNE);
	HAL_UART_Receive(&huart3,Voc_Read_Uart_Buff,9,300);//等待形式读取。
...
//	uint8_t Command_Buff[] = {0x42,0x86,0x04,0x00,0x00,0x00,0x00,0x00,0xff};//读数据命令
	...
	__HAL_UART_CLEAR_OREFLAG(&huart3);
	__HAL_UART_CLEAR_FLAG(&huart3, UART_FLAG_TC);
//	HAL_UART_Transmit(&huart3,Command_Buff,9,10000);//等待形式发送。
	HAL_UART_Transmit_DMA(&huart3,Voc_DMA_Command_Buff,9);//DMA形式发送。
...
//	__HAL_UART_CLEAR_OREFLAG(&huart3);
	__HAL_UART_CLEAR_FLAG(&huart3,UART_FLAG_RXNE);
//	HAL_UART_Receive(&huart3,Voc_Read_Uart_Buff,17,200);//等待形式读取。
	HAL_UART_Receive_IT(&huart3,Voc_Read_Uart_Buff,17);//中断形式读取。
