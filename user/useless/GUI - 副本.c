...
void Gui_Draw_Num32_Temperature(uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, float num)
{
char buf[8];
uint8_t len,i,xStart = 0;
	if(num<0)
		return;
	sprintf(buf,"%.2f",num);
	len = strlen(buf);
	...
	{
		xStart = 24;
		LCD_Clear_Region(x,y,x+26,y+34,BLACK);
	}
	for(i=0;i<len;i++)
	{
		Gui_DrawFont_Num32(xStart+x+i*24, y, fc, bc, buf[i]=='.'? 10 : buf[i]-0x30);
	}
	Gui_DrawFont_Num32(xStart+x+i*24, y, fc, bc, 13);
}

void Gui_Draw_Num32(uint16_t x, uint16_t y, uint16_t fc, uint16_t bc, uint16_t num)
{
char buf[8];
uint8_t len,i,xStart = 0;
	...
	for(i=0;i<len;i++)
	{
		Gui_DrawFont_Num32(xStart+x+i*24, y, fc, bc,buf[i]-0x30);
	}
}
